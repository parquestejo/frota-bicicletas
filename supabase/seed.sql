...
insert into bikes(code,asset_type,model,kiosk_id)
select 'E'||lpad(n::text,3,'0'),'electric','Bicicleta elétrica '||lpad(n::text,3,'0'),(select id from kiosks where name=case when n<=10 then 'Praia da Torre' else 'Terrapleno de Algés' end)
from generate_series(1,20)n on conflict(code) do nothing;
insert into bikes(code,asset_type,model,kiosk_id)
select 'C'||lpad(n::text,3,'0'),'conventional','Bicicleta convencional '||lpad(n::text,3,'0'),(select id from kiosks where name=case when n<=5 then 'Praia da Torre' else 'Terrapleno de Algés' end)
